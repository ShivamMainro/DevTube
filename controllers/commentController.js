...
const getUserComments = async (req, res) => {
    try {
        const channelId = req.channel.id;  // Get logged-in user's channel ID

        // Find all videos posted by the logged-in user
        ...
        // Find all comments on the videos posted by the logged-in user
        const comments = await Comment.find({ video: { $in: videos.map(video => video._id) } })
            .sort({ postedDate: -1 }) // Sort by newest first
            .populate({
                path: 'video',
                select: 'title url'  // Include video title and URL
            })
            .populate({
                path: 'replies',
                populate: {
            ...
            .populate({
                path: 'channel',  // Include the channel information of the commenter
                select: 'logoURL handle'
            });
